...
class ChatConsumer(AsyncWebsocketConsumer):
    async def _after_message_save(self, msg: MessageModel, rid: int, user_pk: str):
        ...
        await self.channel_layer.group_send(
            user_pk,
            OutgoingEventNewUnreadCount(
                sender=self.group_name, unread_count=new_unreads
            )._asdict(),
        )
    ...
    async def connect(self):
        ...
        if self.scope["user"].is_authenticated:
            ...
            logger.info(
                f"Rejecting unauthenticated user with code {UNAUTH_REJECT_CODE}"
            )
            ...
            if msg_type == MessageTypes.IsTyping:
                ...
                if "user_pk" not in data:
                    ...
                    await self.channel_layer.group_send(
                        user_pk,
                        OutgoingEventMessageRead(
                            message_id=mid, sender=user_pk, receiver=self.group_name
                        )._asdict(),
                    )
                    ...
                    if not recipient:
                        ...
                        if not msg_res:
                            ...
                            await self.channel_layer.group_send(
                                self.group_name,
                                OutgoingEventNewUnreadCount(
                                    sender=user_pk, unread_count=new_unreads
                                )._asdict(),
                            )
                ...
                if "file_id" not in data:
                    ...
                    return (
                        ErrorTypes.FileMessageInvalid,
                        "'file_id' should not be blank",
                    )
                    ...
                    if not file:
                        return (
                            ErrorTypes.FileDoesNotExist,
                            f"File with id {file_id} does not exist",
                        )
                    else:
                        recipient: Optional[AbstractBaseUser] = await get_user_by_pk(
                            user_pk
                        ...
                        if not recipient:
                            ...
                            logger.info(
                                f"Will save file message from {self.user} to {recipient}"
                            )
                            msg = await save_file_message(
                                file, from_=self.user, to=recipient
                            )
                            await self._after_message_save(
                                msg, rid=rid, user_pk=user_pk
                            )
                            logger.info(
                                f"Sending file message for file {file_id} from {self.user} to {recipient}"
                            )
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        if error is not None:
            error_data = {"msg_type": MessageTypes.ErrorOccurred, "error": error}
